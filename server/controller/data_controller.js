...
module.exports.fetchUpdatingTask=wrapAsync(async(req,res)=>{
    const {id}=req.params;
...
module.exports.handleStatus=wrapAsync(async(req,res,next)=>{
    const {userId}=req.user;
    const {id}=req.params;
    const task=await Data.findById(id);
...
module.exports.filterTask=wrapAsync(async(req,res,next)=>{
